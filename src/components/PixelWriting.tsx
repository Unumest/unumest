...
import {PixelWritingProps, PixelContainerProps} from '../interfaces/PixelWritingProps';
        ...
        return (
            ...
            >
                {elements.map((element, index) => (
                    <ElementComponent str={element} fontProps={mergedFontProps} key={index} />
                ))}
            </div>
        );
    },
...
const PixelSentence = React.memo((props: PixelWritingProps) => {
    ...
    const CONTAINER_CLASS = 'pixelSentence';
    const mergedFontProps = {...DEFAULT_PIXEL_FONT_PROPS, ...props.fontProps};
    return (
        <PixelContainer
            elements={words}
